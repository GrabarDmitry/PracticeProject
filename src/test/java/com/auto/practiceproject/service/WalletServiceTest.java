...
public class WalletServiceTest {
  ...
  public void setUp() {
    ...
    Mockito.when(walletDAO.findByUser(user)).thenReturn(Optional.of(wallet));
  }
  ...
  public void putMoneyTest() {
    ...
    Mockito.when(walletDAO.findByUser(user)).thenReturn(Optional.of(walletTest));

    walletService.putMoney(5D, user);

    Mockito.verify(walletDAO, Mockito.times(1)).saveAndFlush(walletTest);
  }
  ...
  public void payForServicesTest() {
    ...
    Mockito.verify(walletDAO, Mockito.times(1)).findByUser(user);
    Mockito.verify(walletDAO, Mockito.times(1)).saveAndFlush(any(Wallet.class));
  }
  ...
  public void payForServicesFailTest() {
    ...
    Exception exception =
        assertThrows(WalletOperationException.class, () -> walletService.payForServices(20D, user));
    Assert.assertTrue(
        exception.getMessage().contains("You don't have enough money to carry out the operation!"));

    Mockito.verify(walletDAO, Mockito.times(1)).findByUser(user);
    Mockito.verify(walletDAO, Mockito.times(0)).saveAndFlush(any(Wallet.class));
  }
}
