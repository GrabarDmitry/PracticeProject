...
public class SecurityServiceTest {
  ...
  @MockBean private TokenProvider provider;

  @MockBean private UserDAO userDAO;

  @MockBean private BookmarkDAO bookmarkDAO;

  @MockBean private WalletDAO walletDAO;

  @MockBean private MailService mailService;

  @MockBean private CustomUtil customUtil;
  ...
  public void authenticationTest() {
    ...
    Mockito.when(userDetailsService.loadUserByUsername("dzmitry@mail.ru")).thenReturn(userDetails);
    Mockito.when(provider.createToken(userDetails)).thenReturn("test_token");
    ...
    Mockito.verify(userDetailsService, Mockito.times(1)).loadUserByUsername("dzmitry@mail.ru");
    Mockito.verify(provider, Mockito.times(1)).createToken(userDetails);
  }
  ...
  public void authenticationFailTest() {
    ...
    Assert.assertTrue(exception.getMessage().contains("Invalid Username or password!"));

    Mockito.verify(authenticationManager, Mockito.times(0))
        .authenticate(any(UsernamePasswordAuthenticationToken.class));
    Mockito.verify(userDetailsService, Mockito.times(1)).loadUserByUsername("fail@mail.ru");
    Mockito.verify(provider, Mockito.times(0)).createToken(any(UserDetailsImpl.class));
  }
  ...
  public void registration() {
    ...
    Mockito.doReturn(userTest).when(userDAO).save(any(User.class));

    User user = securityService.registration(userTest);

    Assert.assertNotNull(user);

    Mockito.verify(userDAO, Mockito.times(1)).save(userTest);
    Mockito.verify(walletDAO, Mockito.times(1)).save(any(Wallet.class));
    Mockito.verify(bookmarkDAO, Mockito.times(1)).save(any(Bookmark.class));
  }
  ...
  public void createModeratorUserTest() {
    ...
    Mockito.doReturn(userTest).when(userDAO).save(any(User.class));
    Mockito.doReturn(password).when(customUtil).generatePassword(10);
    Mockito.doReturn("awdad23f2223f").when(passwordEncoder).encode(password);
    ...
    Mockito.verify(customUtil, Mockito.times(1)).generatePassword(10);
    Mockito.verify(mailService, Mockito.times(1)).sendDataToEmail(any(MailDataDTO.class));
  }
}
