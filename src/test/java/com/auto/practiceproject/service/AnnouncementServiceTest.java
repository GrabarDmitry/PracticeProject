...
public class AnnouncementServiceTest {
  ...
  @MockBean private AnnouncementDAO announcementDAO;
  ...
  public void findAllAnnouncementByModerationIsFalse() {
    ...
    Page<Announcement> announcementPage =
        announcementService.findAllAnnouncementByModeration(false, pageable, null);
  ...
  public void announcementCreateTest() {
    Mockito.doReturn(announcementsTestData.get(0))
        .when(announcementDAO)
    ...
    Mockito.verify(announcementDAO, Mockito.times(1)).save(any(Announcement.class));
  ...
  public void findAnnouncementByIdIsNullTest() {
    ...
    Exception exception =
        assertThrows(ResourceException.class, () -> announcementService.findAnnouncementById(3l));
    Assert.assertTrue(exception.getMessage().contains("Announcement with Id: 3 not found"));
  }
  ...
  public void announcementUpdateTest() {
    ...
    Announcement announcement =
        announcementService.updateAnnouncement(announcementsTestData.get(0));

    Mockito.verify(announcementDAO, Mockito.times(1)).findById(1L);
    Mockito.verify(announcementDAO, Mockito.times(1)).saveAndFlush(any(Announcement.class));
  ...
  public void announcementUpdateTestFail() {
    ...
    Exception exception =
        assertThrows(
            ResourceException.class, () -> announcementService.updateAnnouncement(announcement));
    Assert.assertTrue(exception.getMessage().contains("Announcement with Id: 3 not found"));

    Mockito.verify(announcementDAO, Mockito.times(1)).findById(3L);
    Mockito.verify(announcementDAO, Mockito.times(0)).saveAndFlush(any(Announcement.class));
  }
  ...
  public void announcementRatingUpTest() {
    ...
    Mockito.when(announcementDAO.findById(1L)).thenReturn(Optional.of(announcementTest));

    Announcement announcement = announcementService.announcementRatingUp(1L);
  ...
  public void findAnnouncementByUserId() {
    ...
    Page<Announcement> announcementPage =
        announcementService.findAnnouncementByUserId(user, pageable, null);
