...
public class AutoTransmissionServiceTest {
  ...
  @Autowired private AutoTransmissionService autoTransmissionService;

  @MockBean private AutoTransmissionDAO transmissionDAO;
  ...
  public void findAllAutoTransmission() {
    Mockito.when(transmissionDAO.findAll()).thenReturn(autoTransmissionData);
  ...
  public void findAutoTransmissionByIdTest() {
    Mockito.when(transmissionDAO.findById(1l)).thenReturn(Optional.of(autoTransmissionData.get(0)));
  ...
  public void findAutoTransmissionByIdIsNullTest() {
    ...
    Exception exception =
        assertThrows(
            ResourceException.class, () -> autoTransmissionService.findAutoTransmissionById(3L));
    Assert.assertTrue(exception.getMessage().contains("Auto transmission with Id: 3 not found"));
  }
