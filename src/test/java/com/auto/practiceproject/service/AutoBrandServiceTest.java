...
public class AutoBrandServiceTest {
  ...
  @Autowired private AutoBrandService autoBrandService;

  @MockBean private AutoBrandDAO autoBrandDAO;
  ...
  public void findAllAutoBrand() {
    Mockito.when(autoBrandDAO.findAll()).thenReturn(autoBrandData);
  ...
  public void findAutoBrandByIdTest() {
    Mockito.when(autoBrandDAO.findById(1l)).thenReturn(Optional.of(autoBrandData.get(0)));
  ...
  public void findAutoBrandByIdIsNullTest() {
    ...
    Exception exception =
        assertThrows(ResourceException.class, () -> autoBrandService.findAutoBrandById(3L));
    Assert.assertTrue(exception.getMessage().contains("Auto brand with Id: 3 not found"));
  }
}
