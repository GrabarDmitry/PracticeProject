...
public class AutoTransmissionController {
  ...
  @GetMapping("/{id}")
  public ResponseEntity<AutoTransmissionResponseDTO> getAutoTransmissionById(
      @PathVariable Long id) {
    log.trace("Controller method called to view auto transmission with id: {}", id);
    return new ResponseEntity<>(
        converter.toDTO(autoTransmissionService.findAutoTransmissionById(id)), HttpStatus.OK);
  }
}
