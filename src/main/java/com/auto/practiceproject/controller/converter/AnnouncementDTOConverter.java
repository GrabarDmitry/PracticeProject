...
public class AnnouncementDTOConverter {
  ...
  public Announcement toEntity(AnnouncementRequestDTO createDTO) {
    ...
    AutoModel autoModel = autoModelService.findAutoModel(createDTO.getAutoModelId()).orElse(null);
  ...
  public Announcement updateToEntity(Long id, AnnouncementRequestDTO updateDTO) {
    ...
    AutoModel autoModel = autoModelService.findAutoModel(updateDTO.getAutoModelId()).orElse(null);
    ...
    auto.setAutoEngine(autoEngineService.findAutoEngine(updateDTO.getAutoEngineId()).orElse(null));
    auto.setAutoModel(autoModel);
    auto.setAutoTransmission(
        autoTransmissionService
            .findAutoTransmission(updateDTO.getAutoTransmissionId())
            .orElse(null));
  ...
  private Auto createAutoUtil(
      ...
      AutoModel autoModel,
      Long autoEngineId,
      Long autoTransmissionId) {
    return new Auto(
        mileage,
        engineCapacity,
        vim,
        autoModel,
        autoEngineService.findAutoEngine(autoEngineId).orElse(null),
        autoTransmissionService.findAutoTransmission(autoTransmissionId).orElse(null));
  }
}
