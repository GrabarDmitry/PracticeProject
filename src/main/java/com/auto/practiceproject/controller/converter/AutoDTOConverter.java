...
public class AutoDTOConverter {
  ...
  public Auto toEntityCreate(AutoRequestDTO autoRequestDTO) {
    ...
    return new Auto(
        ...
        autoModelService.findAutoModel(autoRequestDTO.getAutoModelId()).orElse(null),
        autoEngineService.findAutoEngine(autoRequestDTO.getAutoEngineId()).orElse(null),
        autoTransmissionService
            .findAutoTransmission(autoRequestDTO.getAutoTransmissionId())
            .orElse(null));
  }

  public Auto toEntityUpdate(Long autoId, AutoRequestDTO autoRequestDTO) {
    log.trace("Convert AutoRequestDTO: {}, to Auto", autoRequestDTO);
    Auto auto = autoService.findAuto(autoId).orElse(null);
    ...
    auto.setAutoModel(autoModelService.findAutoModel(autoRequestDTO.getAutoModelId()).orElse(null));
    auto.setAutoEngine(
        autoEngineService.findAutoEngine(autoRequestDTO.getAutoEngineId()).orElse(null));
